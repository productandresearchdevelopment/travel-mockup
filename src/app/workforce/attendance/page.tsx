  ...
  return (
    <AppShell>
      ...
      <DataTable
        ...
        onExport={() => {
          const headers = "Date,Worker Code,Worker Name,Role,Assignment,Status,Check In,Check Out,Working Hours,Source\n";
          const rows = filteredLogs
            .map((r) => `"${r.date}","${r.workerCode}","${r.workerName}","${r.role}","${r.assignmentName}","${r.status}","${r.checkInTime || ""}","${r.checkOutTime || ""}","${r.workingHours || ""}","${r.source}"`)
            .join("\n");
          const blob = new Blob([headers + rows], { type: "text/csv" });
          const url = window.URL.createObjectURL(blob);
          const a = document.createElement("a");
          a.href = url;
          a.download = "Attendance_Logs_Export.csv";
          a.click();
        }}
